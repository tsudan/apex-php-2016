	...
	<hr />
						...
						if( ! is_numeric( $_POST['price']) ) {
							...
							if( $result ) {
								...
								if (isset($_FILES['book_image'])) {
									...
									$book_id = get_last_added_book_id();

									$book_name = $title . '_' . $isbn;
									$res = upload_book( $_FILES['book_image'], $book_name, $book_id );
		...
		<?php
				foreach( $books as $book ) {
					...
					$upload = "<a href='upload.php?action=upload_book&id={$book['id']}'>update image</a>";
					echo "<tr>"
							. "<td>
								<img src='../img/thumb/{$book['book_image']}' class='book_thumb' />
								<br />
								$upload
							  </td>"
			...
			<?php
				...
				echo 'Books not available at this moment';
			}
